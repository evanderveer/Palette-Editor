...
def parse_args():
    """Parses command line arguments in case the module is called directly."""
...
def build_pal_header(file, file_size):
    """Writes the .pal file header to the file."""

    header = bytes('RIFF', encoding='ascii') + \
             (file_size-8).to_bytes(4, byteorder='little') + \
             bytes('PAL data', encoding='ascii') + \
             (file_size-20).to_bytes(4, byteorder='little') + \
             (0).to_bytes(1, byteorder='little') + \
             (3).to_bytes(1, byteorder='little') + \
             (int((file_size-24)/4)).to_bytes(2, byteorder='little')
    file.write(header)
    
def write_color_list(file, color_list):
    """Writes the list of colors to the .pal file."""
...
def interpolate(x_points, x_data, y_data):
    """Returns interpolated values at the x_points between known x_data and y_data points."""

    interp_points = np.interp(x_points, x_data, y_data)
    return(interp_points)

   
def mainfunction(palette_file, color_list, gradient_steps):
    
    file_size = gradient_steps * 4 + 24 
    color_points = []
    ...
    interp_positions = np.linspace(0, 1, gradient_steps)
...
if __name__ == '__main__':
    ...
    mainfunction(palette_file=args.palette_file, color_list=color_list, gradient_steps=args.gradient_steps)
